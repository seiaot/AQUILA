...
void ShapePlane2D::Init(ID3D12Device* pDevice)
{
	if (pDevice == nullptr)
		return;
...
void ShapePlane2D::Draw(ID3D12GraphicsCommandList* pCmdList){
	m_Mesh.Draw(pCmdList);
}

void ShapePlane2D::SetTexCoord(const XMFLOAT2& left_top, const XMFLOAT2& right_bottom) 
{
...
void ShapePlane2D::SetColor(float r, float g, float b, float a){
...
void ShapePlane2D::SetColor(float r, float g, float b) {
	if (m_pVertex == nullptr)
		return;
...
void ShapePlane2D::SetAlpha(float a) {
	if (m_pVertex == nullptr)
		return;
...
void ShapePlane2D::SetAdditionalColor(float r, float g, float b) {
	if (m_pVertex == nullptr)
		return;

	for (int i = 0; i < 4; ++i) {
		m_pVertex[i].AddColor.x = r;
		m_pVertex[i].AddColor.y = g;
		m_pVertex[i].AddColor.z = b;
	}
}
